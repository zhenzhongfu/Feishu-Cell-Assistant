...
const App: React.FC = () => {
  ...
  const [isAutoSave, setIsAutoSave] = useState(true); // 默认开启自动保存
  const [cellSelected, setCellSelected] = useState(false); // 是否选中单元格
  ...
  return (
    <>
      {(!isBitable || (isBitable && !cellSelected)) ? (
        // 未在飞书环境或未选中单元格时显示产品展示页面
        <LandingPage />
      ) : (
        // 原有的编辑器界面
